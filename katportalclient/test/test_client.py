...
class TestWebSocket(TestWebSocketHandler):
    ...
    def on_message(self, message):
        ...
        elif message["method"] in ("set_sampling_strategy", "set_sampling_strategies"):
            reply["result"] = {}
...
class TestKATPortalClient(WebSocketBaseTestCase):
    def setUp(self):
        ...
        self._portal_client = KATPortalClient(
            SITEMAP_URL, on_msg_callback, io_loop=self.io_loop
        )
    ...
    def test_reconnect(self):
        ...
        self._portal_client._connect = mock.MagicMock(return_value=connect_future)
    ...
    def test_resend_subscriptions(self):
        ...
        self._portal_client._send = mock.MagicMock(return_value=send_future)
    ...
    def test_cache_jsonrpc_request(self):
        ...
        req5 = JSONRPCRequest(
            "set_sampling_strategy", ["namespace", "sensor_name", "strategy_and_params"]
        )
        req6 = JSONRPCRequest(
            "set_sampling_strategy", ["namespace", "sensor_name", "none"]
        )
        req7 = JSONRPCRequest(
        ...
        req8 = JSONRPCRequest(
            "set_sampling_strategies", ["namespace", "sensor_name", "none"]
        )
    ...
    def test_sb_ids_by_capture_block_valid(self):
        ...
        capture_block_base_url = self._portal_client.sitemap["capture_blocks"]
    ...
    def test_sensor_names_multiple_sensors_valid(self):
        ...
        self.mock_http_async_client().fetch.side_effect = self.mock_async_fetcher(
            valid_response='{"data":[%s, %s]}'
            % (
                sensor_json["anc_wind_device_status"],
    ...
    def test_sensor_detail(self):
        ...
        self.mock_http_async_client().fetch.side_effect = self.mock_async_fetcher(
            valid_response=(
                '{{"data":[{}]}}'.format(sensor_json["anc_weather_wind_speed"])
            ),
            invalid_response=sensor_json["invalid_response"],
            starts_with=history_base_url,
    ...
    def test_sensor_detail_for_multiple_sensors_but_exact_match(self):
        ...
        self.mock_http_async_client().fetch.side_effect = self.mock_async_fetcher(
            valid_response='{{"data" : [{}, {}]}}'.format(
                sensor_json["anc_gust_wind_speed2"], sensor_json["anc_gust_wind_speed"]
            ),
            invalid_response="[]",
            starts_with=history_base_url,
            contains=sensor_name_filter,
    ...
    def test_sensor_value_multiple_results_one_match(self):
        ...
        expected_result = SensorSample(
            sample_time=1531302437, value=43680.0, status="nominal"
        )
        ...
        result = yield self._portal_client.sensor_value(
            "anc_tfr_m018_l_band_offset", include_value_ts=True
        )
        expected_result = SensorSampleValueTime(
            sample_time=1531302437,
    ...
    def test_sensor_value_one_result(self):
        ...
        expected_result = SensorSample(
            sample_time=222.222, value=43680.0, status="nominal"
        )
        res = yield self._portal_client.sensor_value(
            "anc_tfr_m018_l_band_offset_average"
        )
        ...
        expected_result = SensorSampleValueTime(
            sample_time=222.222, value_time=111.111, value=43680.0, status=u"nominal"
        )
        res = yield self._portal_client.sensor_value(
            "anc_tfr_m018_l_band_offset_average", include_value_ts=True
        )
        assert res == expected_result
    ...
    def test_sensor_values_one_filter_multiple_matches(self):
        ...
        expected_result = {
            "anc_tfr_m018_l_band_offset": SensorSample(
                sample_time=1531302437, value=43680.0, status="nominal"
            ),
            "some_other_sample": SensorSample(
                sample_time=222.222, value=43680.0, status="nominal"
            ),
        }
        assert result == expected_result
        ...
        result = yield self._portal_client.sensor_values(
            "ARBITRARY_FILTER", include_value_ts=True
        )
        expected_result = {
            "anc_tfr_m018_l_band_offset": SensorSampleValueTime(
                sample_time=1531302437,
                value_time=1530713112,
                value=43680.0,
                status="nominal",
            ),
            "some_other_sample": SensorSampleValueTime(
                sample_time=222.222, value_time=111.111, value=43680.0, status="nominal"
            ),
        }
        assert result == expected_result
    ...
    def test_sensor_values_multiple_filters_multiple_matches(self):
        ...
        expected_result = {
            "some_other_sample_0": SensorSample(
                sample_time=220.222, value=43480.0, status="nominal"
            ),
            "some_other_sample_1": SensorSample(
                sample_time=221.222, value=43580.0, status="nominal"
            ),
        }
        res = yield self._portal_client.sensor_values(
            ["some_other_sample_0", "some_other_sample_1"]
        )
    ...
    def test_sensor_history_multiple_sensors_valid_times(self):
        ...
        self.mock_http_async_client().fetch.side_effect = self.mock_async_fetchers(
            valid_responses=[
                '{{"data" : [{}, {}]}}'.format(
                    sensor_json[sensor_names[0]], sensor_json[sensor_names[1]]
                ),
                sensor_data2,
                sensor_data1,
            ],
            ...
            containses=[sensor_name_filter, sensor_names[0], sensor_names[1]],
        )
    ...
    def test_sensor_history_multiple_sensor_futures(self):
        ...
        self.mock_http_async_client().fetch.side_effect = self.mock_async_fetchers(
            valid_responses=[sensor_data4, sensor_data3],
            invalid_responses=["1error", "2error"],
            starts_withs=history_base_url,
            containses=[sensor_names[0], sensor_names[1]],
        )

        futures = []
        futures.append(
            self._portal_client.sensor_history(
    ...
    def test_login(self):
        ...
        self.assertEquals(
            self._portal_client._session_id, "token generated by katportal"
        )
    ...
    def test_create_userlog(self):
        ...
        self.assertEquals(
            userlog,
            {
                ...
                u"attachments": u"[]",
                u"tags": u"[]",
                u"timestamp": u"2017-02-07 08:47:22",
                u"start_time": u"2017-02-07 00:00:00",
                u"modified": u"",
                u"content": u"test content",
                u"parent_id": u"",
                u"user": {u"email": u"cam@ska.ac.za", u"name": u"CAM", u"id": 1},
                u"attachment_count": u"0",
                u"id": u"40",
    ...
    def test_sensor_subarray_lookup(self):
        ...
        lookup_base_url = (
            self._portal_client.sitemap["subarray"]
    ...
    def mock_async_fetchers(
        ...
        mock_fetches = [
            self.mock_async_fetcher(v, i, s, e, c)
            for v, i, s, e, c in zip(
                valid_responses, invalid_responses, starts_withs, ends_withs, containses
            )
        ]
